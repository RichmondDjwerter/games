    ...
    while guesses <= max:
        ...
        if len(user_input) != 1:
            if user_input == ''.join(word_to_guess):
                print('congratulations, you won! The word was:',''.join(word_to_guess))
                break
            else:
            ...
            if user_input in word_to_guess:
                print("Good! You guessed the letter right.")
            ...
            if '_' not in currently:
                print('Congratulations! You win. The word was:',''.join(word_to_guess))
                break
    if (guesses >= max):
        print('Sorry, you lost! The word was:',''.join(word_to_guess))
